...
#ws.geometry('400x300') #width x height
ws.state('zoomed')
#ws.minsize(250,190)
#ws.config(bg='#686e70')
ws.configure(bg='powder blue')

var = IntVar()

frame = Frame(ws, padx=10, pady=10)
frame.configure(bg = 'powder blue') #-----------
frame.pack(expand=True)

age_lb = Label(frame, text="Enter Age (2 - 120)", bg = 'powder blue')
age_lb.grid(row=1, column=1)

age_tf = Entry(frame, )
age_tf.grid(row=1, column=2, pady=5)

gen_lb = Label(frame, text='Select Gender' , bg = 'powder blue')
...
height_lb = Label(frame, text="Enter Height (cm)  ", bg = 'powder blue')
height_lb.grid(row=3, column=1)

weight_lb = Label(frame, text="Enter Weight (kg)  ", bg = 'powder blue')
...
cal_btn = Button(frame3, text='Calculate', bg = 'yellow', fg = 'red' , command=calculate_bmi)
